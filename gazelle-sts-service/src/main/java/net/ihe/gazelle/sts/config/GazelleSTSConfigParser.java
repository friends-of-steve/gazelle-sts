...
/**
 * <p>
 * A Stax parser for the Gazelle STS configuration.
 * </p>
 *
 * @author <a href="mailto:sguilhen@redhat.com">Stefan Guilhen</a>, Updated by cel on 18/04/17 (Unfortunatly, due to
 *         bad picketlink design, the original class STSConfigParser cannot be properly extended to add element in config file,
 *         so the full parser need to be rewrite with gazelle elements).
 * @version $Id: $Id
 */
public class GazelleSTSConfigParser extends AbstractParser {

    // XML configuration elements.
    /**
     * Constant <code>ROOT_ELEMENT="PicketLinkSTS"</code>
     */
    ...
    //Gazelle Additions
    private static final String CERT_VAL_ENDPOINT_ELEMENT = "CertificateValidatorEndPoint";

    private static final String ISSUER_TRUST_VALIDATION_ELEMENT = "IssuerTrustValidation";
    ...
    public Object parse(XMLEventReader xmlEventReader) throws ParsingException {
        ...
        GazelleSTSType configType = new GazelleSTSType();
        ...
        while (xmlEventReader.hasNext()) {
            ...
            } else if (ISSUER_TRUST_VALIDATION_ELEMENT.equals(elementName)) {
                configType.setIssuerTrustValidation(parseIssuerTrustValidation(xmlEventReader));
            } else {
    ...
    private IssuerTrustValidation parseIssuerTrustValidation(XMLEventReader xmlEventReader) throws ParsingException {
        StartElement startElement = StaxParserUtil.getNextStartElement(xmlEventReader);
        StaxParserUtil.validate(startElement, ISSUER_TRUST_VALIDATION_ELEMENT);

        IssuerTrustValidation issuerTrustValidation = new IssuerTrustValidation();

        QName attributeQName = new QName("", ENABLED_ATTRIB);
        Attribute attribute = startElement.getAttributeByName(attributeQName);
        String attributeValue = StaxParserUtil.getAttributeValue(attribute);

        issuerTrustValidation.setIssuerTrustValidation(attributeValue);
        ...
        while (xmlEventReader.hasNext()) {
            ...
            if (xmlEvent instanceof EndElement) {
                EndElement endElement = (EndElement) StaxParserUtil.getNextEvent(xmlEventReader);
                String endElementName = StaxParserUtil.getEndElementName(endElement);
                if (endElementName.equals(ISSUER_TRUST_VALIDATION_ELEMENT)) {
                    break;
                } else {
                    throw logger.parserUnknownEndElement(endElementName);
                }
            ...
            if (CERT_VAL_ENDPOINT_ELEMENT.equalsIgnoreCase(elementName)) {
                subEvent = StaxParserUtil.getNextStartElement(xmlEventReader);
                if (!StaxParserUtil.hasTextAhead(xmlEventReader)) {
                    throw new ParsingException(ErrorCodes.EXPECTED_TEXT_VALUE + CERT_VAL_ENDPOINT_ELEMENT);
                }
                issuerTrustValidation.setPKICertificateValidatorEndPoint(StaxParserUtil.getElementText(xmlEventReader));
            }
            //TODO RSAKeyValueConfig
        }
